...
Copyright (C) 2016 Bogdan Marinescu (mbed port)
...
/* Adapted from https://github.com/adafruit/ST7565-LCD. Changes:
 *
 * fixes to work with the EA display (different page map, byte mirroring).
 * uses the init sequence from the EA datasheet.
 * uses hardware SPI.
 * partial update mode disabled for now
 * many others
 */
...
void ST7565::putbuffer(const uint8_t *bitmap, bool invert) {
    uint8_t d;

    for (unsigned i = 0; i < sizeof(st7565_buffer); i ++) {
        d = bitmap[i];
        st7565_buffer[i] = invert ? (uint8_t)(~d) : d;
    }
...
void  ST7565::drawchar(uint8_t x, uint8_t line, char c) {
    unsigned line_offset = line * LCDWIDTH;
...
uint8_t ST7565::getpixel(uint8_t x, uint8_t y) {
    ...
    return (st7565_buffer[x+ (y/8)*128] >> ((y%8))) & 0x1;
}
...
void ST7565::st7565_init(void) {
    // Initialization sequence is copied verbatim from the datasheet.
    uint8_t init_data[] = {0x40, 0xA1, 0xC0, 0xA6, 0xA2, 0x2F, 0xF8, 0x00, 0x27, 0x81, 0x16, 0xAC, 0x00, 0xAF};
    ...
    for (unsigned i = 0; i < sizeof(init_data) / sizeof(uint8_t); i ++)
        st7565_command(init_data[i]);
}
