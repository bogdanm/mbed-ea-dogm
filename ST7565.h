...
class ST7565 {
public:
    ST7565(PinName mosi, PinName sclk, PinName A0, PinName RST, PinName CS):
        spi(mosi, NC, sclk), a0(A0), rst(RST), cs(CS) {}
    ...
    void putbuffer(const uint8_t *bitmap, bool invert = false);

private:
    SPI spi;
    DigitalOut a0, rst, cs;
